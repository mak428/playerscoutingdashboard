...
MARKET_VALUE_FORECAST_FILE = "/Users/mukikrishnan/Desktop/Interactive Soccer Dashboard/Market Value Forecast Data/ensemble_player_forecasts_5y_adjusted_full.csv"

# Helper function to format values like 16000000 -> 16M
...
page = st.sidebar.radio("Go to", ["Home", "Basic Filter Search", "Player Comparison", "Player Archetype", "Transfer Market Value Predictions", "Player Shortlist"])
...
# Basic Filter Search Page
    ...
    filtered_df = df.copy()
    if player_name:
        search_norm = normalize_text(player_name)
        filtered_df = filtered_df[filtered_df['Player'].apply(lambda x: search_norm in normalize_text(x))]
    if league != 'All':
        filtered_df = filtered_df[filtered_df['Comp'].apply(lambda x: normalize_text(x) == normalize_text(league))]
    if team != 'All':
        filtered_df = filtered_df[filtered_df['Team'].apply(lambda x: normalize_text(x) == normalize_text(team))]
...
# Player Archetype Page
    ...
    if position == "Goalkeeper":
        ...
        if player_search:
            search_norm = normalize_text(player_search)
            filtered_archetypes = filtered_archetypes[filtered_archetypes['Player'].apply(lambda x: search_norm in normalize_text(x))]
        if filtered_archetypes.empty:
            st.warning("No players found for this archetype and search term. Clear the search or choose a different archetype.")
    ...
    # Select an archetype for defenders
        ...
        archetype = st.selectbox('Select Archetype', ['Classic CM', 'Regista', 'Box-to-Box', 'Segundo Volante', 'Mezzala', 'Deep Lying Playmaker', 'Ball Winning Midfielder', 'Anchor Man', 'Attacking Midfielder', 'Advanced Playmaker', 'Wide Midfielder', 'Wide Playmaker'])
...
# Transfer Market Value Predictions Page
elif page == "Transfer Market Value Predictions":
    st.title("Transfer Market Value Predictions")

    @st.cache_data
    def load_market_value_forecasts(file_path):
        df = pd.read_csv(file_path)

        df["Forecast Date"] = pd.to_datetime(df["Forecast Date"], errors="coerce")
        df["Predicted Market Value"] = pd.to_numeric(
            df["Predicted Market Value"], errors="coerce"
        )

        return df.dropna(subset=["Forecast Date", "Predicted Market Value"])

    mv_df = load_market_value_forecasts(MARKET_VALUE_FORECAST_FILE)

    st.write("""
    This page shows projected player market values over the next 5 years.
    You can search for a player, view their forecast trend, and inspect the model used.
    """)

    # Filters
    player_search = st.text_input("Search for a player")

    filtered_mv_df = mv_df.copy()

    if player_search:
        search_norm = normalize_text(player_search)
        filtered_mv_df = filtered_mv_df[
            filtered_mv_df["Player"].apply(lambda x: search_norm in normalize_text(x))
        ]

    player_options = sorted(filtered_mv_df["Player"].dropna().unique())
    ...
    if len(player_options) == 0:
        ...
        player_forecast = mv_df[mv_df["Player"] == selected_player].copy()
        player_forecast = player_forecast.sort_values("Forecast Date")

        latest_row = player_forecast.iloc[0]
        final_row = player_forecast.iloc[-1]

        col1, col2, col3, col4 = st.columns([1.2, 1.2, 2.2, 1.8])

        col1.metric(
            "Current Forecast",
            format_market_value(latest_row["Current Market Value"])
        )

        col2.metric(
            "5-Year Forecast",
            format_market_value(final_row["Forecasted Market Value"])
        )

        col3.markdown("### Last Known Team")
        col3.write(f"**{latest_row['Last Known Team']}**")

        col4.markdown("### Model Used")
        col4.write(f"**{latest_row['Best Model']}**")

        # Plot market value forecast
        fig = px.line(
            player_forecast,
            x="Forecast Date",
            y="Forecasted Market Value",
            title=f"{selected_player} - 5 Year Market Value Forecast",
            markers=True,
        )
        ...
        st.dataframe(player_forecast)

        # Optional download button
        csv = player_forecast.to_csv(index=False).encode("utf-8")

        st.download_button(
            label="Download This Player's Forecast",
            data=csv,
            file_name=f"{selected_player}_market_value_forecast.csv",
            mime="text/csv",
        )
